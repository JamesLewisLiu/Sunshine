...
TEST(WindowsTouchRotationTest, Clockwise90ProducesSdvxCompensationCorners) {
...
TEST(WindowsTouchRotationTest, Clockwise90CancelsObservedSdvxCoordinateTransform) {
  ...
  // SDVX reports an uncorrected point as (y, 1-x). Applying the compensation first restores the client point.
  const auto sdvx_x = injected_y;
  const auto sdvx_y = 1.0f - injected_x;
  EXPECT_FLOAT_EQ(sdvx_x, client_x);
  EXPECT_FLOAT_EQ(sdvx_y, client_y);
}
